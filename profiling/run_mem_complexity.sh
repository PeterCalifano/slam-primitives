...
#
# @file run_mem_complexity.sh
# @authors Davide Perico (davide.perico@polimi.it), Pietro Califano (petercalifano.gs@gmail.com)
# @brief Script to run memory complexity analysis via valgrind massif
# @date 2023-03-30
#
...
function usage() {
    usage_parser
    echo "C++ Memory Complexity Profiling Tool"
    echo ""
    echo "Runs valgrind --tool=massif on the target executable and generates"
    echo "human-readable reports via ms_print."
}
...
echo -e "\033[1;34m[INFO] Starting C++ memory complexity profiling tool...\033[0m"
...
echo -e "\033[1;34m[INFO] C++ memory complexity profiling tool completed.\033[0m"
